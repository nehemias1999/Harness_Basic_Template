...
2. Por cada REQ aprobado, en su frontmatter: `estado: draft` → `estado: aprobado`,
   `aprobado_el:` con la fecha de hoy (formato `AAAA-MM-DD`), y `actualizado:`
   igual. Añade la fila de la bitácora (§8): "aprobado por \<humano\>".
3. **Firma el contenido**: calcula la huella del spec y escríbela en
   `aprobado_hash`:

   ```bash
   python scripts/validate_requirements.py --huella specs/REQ-00N_nombre.md
   ```

   Esto es lo que hace que "aprobado" signifique *aprobé este texto* y no
   *alguien escribió la palabra*: si después se editan los criterios, el
   verificador lo detecta. Hazlo **después** de la fila de la bitácora — §7 y
   §8 quedan fuera de la huella justamente para poder tocarlas luego.
4. Por cada feature de esos REQ en `status: "draft"` → `"pending"`. Ninguna
   otra feature se toca, ningún otro campo se toca.
5. Si esta ronda incluía `docs/architecture.md`: borra su nota inicial
...
6. Entrada en `progress/history.md` con los REQ aprobados y las features
   promovidas.
7. Verificador. Tiene que quedar verde salvo el `[WARN]` de 0 tests. Si la
   sección 5 falla, **el estado quedó a medias**: lee el mensaje y termina el
   paso que falte antes de decir nada.
8. Dile al humano cuántas features quedaron `pending` y **cuál es la primera**
