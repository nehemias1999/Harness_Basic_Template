...
TEMPLATE_URL = "Harness_Basic_Template"
...
class Instantiator:
    ...
    def configure_git(self) -> str | None:
        """Returns a closing note for the human, if one is needed.

        Two reasons to touch git here: the reviewer identifies the files touched
        by comparing against history — with no repo it works blind — and if you
        cloned the template, `origin` still points at it and your first push
        would send the new project to the template's repository.
        """
        if self.args.no_git:
        ...
        if not has_git:
            ...
            commit = self.git("commit", "--quiet", "-m", f"chore: instantiate the harness for {self.args.name}")
        ...
        if origin and TEMPLATE_URL in origin:
            self.git("remote", "remove", "origin")
            ok(f"git: remote 'origin' -> {origin} (it was the template, disconnected)")
            return "Add your project's remote: git remote add origin <url>"
        if origin:
            ok(f"git: remote 'origin' -> {origin} (not the template, left alone)")
        else:
            ok("git: existing repository with no remote, nothing to disconnect")

        if self.git("log", "--oneline", "-1").returncode == 0 and not self.args.reset_git:
            warn("git: you are keeping the template's history. Use --reset-git to start clean.")
        return None
